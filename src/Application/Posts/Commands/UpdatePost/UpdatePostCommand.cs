    ...
    : IRequestHandler<UpdatePostCommand>
{
    ...
    private async Task DeleteFilesIfNotExisted(UpdatePostCommand request, CancellationToken cancellationToken, Post entity)
    {
        ...
        var removedFileStorageInfo = oldFileStorageInfo.Where(x => !newFileStorageInfoKeys.Contains(x.StorageKey)).ToArray();
        await fileStorageService.DeleteAllAsync(removedFileStorageInfo.Select(x => x.StorageKey).ToArray(), FileCategory.Post, cancellationToken);
    }
}
